...
class FillThread extends Thread {
	...
	public void run() {
		while(true) {
			try {
				sleep(200);
				bar.fill(); // 0.2초마다 바를 1씩 줄인다.
			} catch (InterruptedException e) { return; }
		}
	}
}
public class Q121JC19011447 extends JFrame{
	private MyLabel bar = new MyLabel(100); // 바의 최대 크기를 100으로 설정
	public Q121JC19011447(String title) {
		super(title);
		...
		th.start(); // 스레드 시작
		FillThread th2 = new FillThread(bar); // 스레드 생성
		th2.start(); // 스레드 시작
	}
	public static void main(String[] args) {
		new Q121JC19011447("12-1 퀴즈");
